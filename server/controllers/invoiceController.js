...
export const generateInvoice = async (req, res) => {
    ...
    const templatePath = path.join(process.cwd(), "utils", "invoiceTemplate.xlsx");
    ...
    sheet.getCell("M23").value = Number(postVAT);
    sheet.getCell("M23").numFmt = '#,##0.00';
    sheet.getCell("M27").value = Number(postVAT);
    sheet.getCell("M27").numFmt = '#,##0.00';

    // TAX Deduction // (IVA Retenido)
    sheet.getCell("N23").value = Number(deduction);
    sheet.getCell("N23").numFmt = '#,##0.00';
    sheet.getCell("N27").value = Number(deduction);
    sheet.getCell("N27").numFmt = '#,##0.00';

    // Save copy server-side
    const dir = `invoices/${enterprise.entName.replace(/[\/\\?%*:|"<>]/g, "_")}/${emission.getFullYear()}`;
